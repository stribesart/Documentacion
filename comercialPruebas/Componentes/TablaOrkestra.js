...
export function Table(json, id, sizeClass, ACampoId ,  type = 0, colspanEncabezados = 0){
 ...
 $card.className = `card card-table border-light shadow-sm mb-4`;
  ...
  contenido.forEach(registro => {
   ...
   for(let i = 0; i <= tamanioTabla; i++){
    ...
    if(i === tamanioTabla && type === 0){
     ...
     $td.appendChild(Button('<i class="fas fa-user"></i>','seleccionar', 'btn-primary', `data-selected-${id}`, registro[registro.length-1]));
    }else{
